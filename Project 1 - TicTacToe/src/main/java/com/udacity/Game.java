...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        String result = "None";
            if (grid[0][0]!='-' && grid[0][0]==grid[1][0] && grid[1][0]==grid[2][0]) result=tellTheWinner(grid[0][0]);
            if (grid[0][1]!='-' && grid[0][1]==grid[1][1] && grid[1][1]==grid[2][1]) result=tellTheWinner(grid[0][1]);
            if (grid[0][2]!='-' && grid[0][2]==grid[1][2] && grid[1][2]==grid[2][2]) result=tellTheWinner(grid[0][2]);
            if (grid[0][0]!='-' && grid[0][0]==grid[0][1] && grid[0][1]==grid[0][2]) result=tellTheWinner(grid[0][0]);
            if (grid[1][0]!='-' && grid[1][0]==grid[1][1] && grid[1][1]==grid[1][2]) result=tellTheWinner(grid[1][0]);
            if (grid[2][0]!='-' && grid[2][0]==grid[2][1] && grid[2][1]==grid[2][2]) result=tellTheWinner(grid[2][0]);
            if (grid[0][0]!='-' && grid[0][0]==grid[1][1] && grid[1][1]==grid[2][2]) result=tellTheWinner(grid[0][0]);
            if (grid[2][0]!='-' && grid[2][0]==grid[1][1] && grid[1][1]==grid[0][2]) result=tellTheWinner(grid[2][0]);
            if (freeSpots==0&&result=="None") result = "Tie";
            return result;
    }

    public String tellTheWinner(char player){
        return Character.toString(player).toUpperCase() + " wins";
    }
